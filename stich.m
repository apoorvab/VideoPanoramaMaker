function newImg = stich(imcyl1, imcyl2, Tx, Ty)
% ===============================================================================
% PURPOSE:          
% CREATED:          
%
% imcyl1:           Height x Width x RGB
% imcyl2:           Height x Width x RGB
% Tx:               Translation in x
% Ty:               Translation in y
% =============================================================================== 
...
% that all images are of the same sizes
...
for y = 1:size(imcyl2, 1)
   for x = 1:size(imcyl2, 2)
       
       if ((Tx+x) > 0 && (Ty+y) >0)
           % if we are in the overlap region, then we need to blend.
           if ( x < xoverlap &&  (Ty + y) <= ydimfirst)
               % if both images contain non-black pixels,
               if ( (imcyl1(Ty + y, Tx + x, 1) ~= 0 || imcyl1(Ty + y, Tx + x, 2) ~= 0 || imcyl1(Ty + y, Tx + x, 3) ~= 0) && ....                   
                    (imcyl2(y, x, 1) ~= 0 || imcyl2(y, x, 2) ~= 0 || imcyl2(y, x, 3) ~= 0) )          
                   % then blend them
               ...
               elseif ( imcyl1(Ty + y, Tx + x, 1) == 0 && imcyl1(Ty + y, Tx + x, 2) == 0 && imcyl1(Ty + y, Tx + x, 3) == 0 )
                   % then, pass through second image.
                   newImg(Ty + y, Tx + x, :) = imcyl2(y, x, :);
               % else if second image contains black pixel
               elseif ( imcyl2(y, x, 1) == 0 && imcyl2(y, x, 2) == 0 && imcyl2(y, x, 3) == 0 )
                   % then, pass through first image.
                   newImg(Ty + y, Tx + x, :) = imcyl1(Ty + y, Tx + x, :);
               % else => both images contain black pixels.
               else
                   % so we don't really need to do anything. but all the same 
                   newImg(Ty + y, Tx + x, :) = [0 0 0];
               end
           else % if we are not in the overlap region, we just pass the pixel through from second image
